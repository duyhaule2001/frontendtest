...
import { updatePaidTech } from '../../../../services/hr.service';

const UpdateTechPaidLeave = ({
    openModalUpdateTechPaid,
    updateTechPaidLeave,

    setOpenModalUpdateTechPaid,
    setOpenTechView,
    fetchPaidLeaveManagement,
}) => {
    const [form] = Form.useForm();
    const [fileList, setFileList] = useState([]);
    const [previewImage, setPreviewImage] = useState(updateTechPaidLeave?.file || '');

    useEffect(() => {
        if (updateTechPaidLeave && updateTechPaidLeave.date) {
            form.setFieldsValue({
                ...updateTechPaidLeave,
                date: dayjs(updateTechPaidLeave.date),
            });
            setPreviewImage(updateTechPaidLeave.file || '');
        }
    }, [updateTechPaidLeave, openModalUpdateTechPaid]);
    ...
    const onFinish = async (values) => {
        try {
            ...
            const res = await updatePaidTech(updateTechPaidLeave.id, formData);
            if (res.data) {
                ...
                setOpenModalUpdateTechPaid(false);
                fetchPaidLeaveManagement();
...
export default UpdateTechPaidLeave;
